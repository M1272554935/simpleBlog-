<template>
  <div>
    ...
    <div style="text-align: left">
      ...
      <input v-model="articleTitle" type="text" class="titleInp" placeholder="标题需小于50字符"></input>

      <el-tag effect="dark">文章简介:</el-tag>
      <input v-model="articleInfo" type="text" class="infoInp" placeholder="简介需小于150字符"></input>
    </div>
    ...
    <div class="submit_btn">
      ...
      <el-button type="success" @click="draft">存为草稿</el-button>
...
<script>
    ...
    export default {
      ...
      methods: {
        ...
        checkArticle(){
          if (this.articleContent.length === 0 || this.articleContent == ""){
            this.$message.error('内容不能为空！');
            return false
          }
          if (this.type.length === 0 || this.type == ""){
            this.$message.error('请选择文章类型！');
            return false
          }
          if (this.articleInfo.length === 0 || this.articleInfo == ""){
            this.$message.error('文章简介不能为空！');
            return false
          }
          if (this.articleTitle.length === 0 || this.articleTitle == ""){
          ...
          if (this.this.articleTitle.length >50){
            this.$message.error('标题需小于50个字符！');
          }
          if (this.this.articleInfo.length >150){
            this.$message.error('简介需小于150个字符！');
          }
        },
        //发布文章
        submit(){
        ...
        draft(){
          this.checkArticle();
          var qs = require('qs');
          this.$axios.post('/api/admin/push/draft',qs.stringify(
            {
              content: this.articleContent,
              type: this.type,
              title: this.articleTitle,
        ...
        imgAdd(pos, $file){
          ...
          this.$axios.post("/api/admin/uploadImg",formdata).then((res) => {
            ...
            this.$refs.md.$img2Url(pos, res.data.imaUrl) //回显
          })
        },
